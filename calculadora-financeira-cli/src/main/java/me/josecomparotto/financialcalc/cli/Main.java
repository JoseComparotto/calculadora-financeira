...
public class Main {
    ...
    private static BigDecimal readBigDecimal(Scanner sc, String label) {
        while (true) {
            try {
    ...
    private static int readPrecision(Scanner sc) {
        while (true) {
    ...
    private static BigDecimal readPercentAsRate(Scanner sc, String label) {
        while (true) {
            try {
                System.out.print(label + ": ");
    ...
    private static void menuJurosCompostos(Scanner sc) {
        System.out.println("\n== Juros Compostos ==");
        BigDecimal principal = readBigDecimal(sc, "Principal");
        BigDecimal taxa = readPercentAsRate(sc, "Taxa em % (ex.: 10 para 10%)");
        int tempo = readInt(sc, "Tempo (períodos)");
    ...
    private static void menuParcelasSemJuros(Scanner sc) {
        System.out.println("\n== Parcelas Sem Juros ==");
        BigDecimal principal = readBigDecimal(sc, "Valor Principal");
        int n = readInt(sc, "Número de parcelas");
    ...
    private static void menuParcelasSac(Scanner sc) {
        System.out.println("\n== Parcelas SAC ==");
        BigDecimal principal = readBigDecimal(sc, "Valor Principal");
    ...
    private static void menuParcelasPrice(Scanner sc) {
        System.out.println("\n== Parcelas PRICE ==");
        BigDecimal principal = readBigDecimal(sc, "Valor Principal");
        BigDecimal taxa = readPercentAsRate(sc, "Taxa em % por periodo (ex.: 10)");
        int n = readInt(sc, "Numero de parcelas");
    ...
    private static void imprimirParcelas(List<Parcela> parcelas) {
        ...
        // Usa a escala da ultima parcela para exibir os totais
