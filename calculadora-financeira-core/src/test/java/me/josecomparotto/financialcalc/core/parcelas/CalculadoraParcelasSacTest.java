...
public class CalculadoraParcelasSacTest {
    ...
    private final CalculadoraParcelasSac calc = new CalculadoraParcelasSac(mc);
    ...
    void geraCalendarioSac_basico() {
        ...
        List<Parcela> ps = calc.calcularParcelas(principal, taxa, n, prec);
        assertEquals(n, ps.size());

        BigDecimal amortConst = principal.divide(BigDecimal.valueOf(n), mc).setScale(prec, RoundingMode.HALF_UP);
        for (int i = 0; i < n; i++) {
            Parcela p = ps.get(i);
            assertEquals(i + 1, p.getSerie());
            assertEquals(amortConst, p.getValorAmortizacao());
            BigDecimal expectedSaldo = principal.subtract(amortConst.multiply(BigDecimal.valueOf(i + 1))).setScale(prec, RoundingMode.HALF_UP);
            assertEquals(expectedSaldo, p.getSaldoDevedor());
            // juros = saldo anterior * taxa
            BigDecimal saldoAnterior = i == 0 ? principal : principal.subtract(amortConst.multiply(BigDecimal.valueOf(i))).setScale(prec, RoundingMode.HALF_UP);
            BigDecimal expectedJuros = saldoAnterior.multiply(taxa).setScale(prec, RoundingMode.HALF_UP);
    ...
    void numeroParcelasZero_ouNegativo_deveLancar() {
        ...
        assertThrows(IllegalArgumentException.class, () -> calc.calcularParcelas(principal, taxa, 0, 2));
        assertThrows(IllegalArgumentException.class, () -> calc.calcularParcelas(principal, taxa, -1, 2));
    }
